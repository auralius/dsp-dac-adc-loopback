...
def plot_results(results: list[SweepResult], out_dir: Path | None = None, show: bool = True) -> None:
    ...
    phase_deg = np.asarray([r.phase_deg for r in results], dtype=np.float64)
    ...
    plt.figure()
    plt.semilogx(f, phase_deg, "o-")
    plt.grid(True, which="both")
    plt.xlabel("Frequency [Hz]")
    plt.ylabel("Phase [deg]")
    plt.title("Measured Filter Phase Response")
    if out_dir is not None:
        plt.savefig(out_dir / "phase_response.png", dpi=160, bbox_inches="tight")
    ...
    with serial.Serial(args.port, args.baud, timeout=5) as ser:
        ...
        for f_cmd in freqs:
            ...
            print(
                ...
                f"phase={result.phase_deg:.3f} deg, "
